...
class galpropDATA:
	def __init__(self,fitsdir, galdefid, parser, Rsun=8.5):
		...
		if ((parser=="spectra") or (parser=="abundances") or (parser=="ratios")):
			...
			R = (numpy.arange(int(hdu[0].header["NAXIS1"]))) * hdu[0].header["CDELT1"] + hdu[0].header["CRVAL1"]
			inds = []
			weights = []
			if (R[0] > self.Rsun):
				inds.append(0)
			...
			self.energy = 10**(float(hdu[0].header["CRVAL3"]) + numpy.arange(int(hdu[0].header["NAXIS3"]))*float(hdu[0].header["CDELT3"]))
			...
			for i in range(1, Nnuclei+1):
				...
				if K not in self.density[Z][A]:
					self.density[Z][A][K] = []
				d = ( (data[i-1,:,0,inds].swapaxes(0,1)) * numpy.array(weights) ).sum(axis=1)
		...
		if (parser=="synchrotron"):
			...
			if (os.path.exists(os.path.join(fitsdir,"synchrotron_skymap_"+galdefid))):
				...
				self.synchrotron["total"] = sf[0].data.sum(axis=3).sum(axis=2)[0]/sf[0].data.shape[2]/sf[0].data.shape[3]
				self.synchrotron["nu"] = 10**(float(sf[0].header["CRVAL3"]) + numpy.arange(int(sf[0].header["NAXIS3"]))*float(sf[0].header["CDELT3"]))
				if (os.path.exists(os.path.join(fitsdir,"synchrotron_Q_skymap_"+galdefid))):
					sf = pyfits.open(os.path.join(fitsdir,"synchrotron_Q_skymap_"+galdefid))
					self.synchrotron["Q"] = sf[0].data.sum(axis=3).sum(axis=2)[0]/sf[0].data.shape[2]/sf[0].data.shape[3]
					sf = pyfits.open(os.path.join(fitsdir,"synchrotron_U_skymap_"+galdefid))
					self.synchrotron["U"] = sf[0].data.sum(axis=3).sum(axis=2)[0]/sf[0].data.shape[2]/sf[0].data.shape[3]
				...
				self.synchrotron["total"] = sf[0].data.sum(axis=2).sum(axis=1)/sf[0].data.shape[1]/sf[0].data.shape[2]
				self.synchrotron["nu"] = sf[1].data.field(0)
				if (os.path.exists(os.path.join(fitsdir,"synchrotron_Q_mapcube_"+galdefid))):
					sf = pyfits.open(os.path.join(fitsdir,"synchrotron_Q_mapcube_"+galdefid))
					self.synchrotron["Q"] = sf[0].data.sum(axis=2).sum(axis=1)/sf[0].data.shape[1]/sf[0].data.shape[2]
					sf = pyfits.open(os.path.join(fitsdir,"synchrotron_U_mapcube_"+galdefid))
					self.synchrotron["U"] = sf[0].data.sum(axis=2).sum(axis=1)/sf[0].data.shape[1]/sf[0].data.shape[2]
				...
				self.synchrotron["total"] = sf[1].data.field(0).sum(axis=0)/float(sf[1].header["NAXIS2"])
				self.synchrotron["nu"] = sf[2].data.field(0)
				if (os.path.exists(os.path.join(fitsdir,"synchrotron_Q_healpix_"+galdefid))):
					sf = pyfits.open(os.path.join(fitsdir,"synchrotron_Q_healpix_"+galdefid))
					self.synchrotron["Q"] = sf[1].data.field(0).sum(axis=0)/float(sf[1].header["NAXIS2"])
					sf = pyfits.open(os.path.join(fitsdir,"synchrotron_U_healpix_"+galdefid))
					self.synchrotron["U"] = sf[1].data.field(0).sum(axis=0)/float(sf[1].header["NAXIS2"])
		...
		if (parser=="gamma"):
			...
			if ( os.path.exists(os.path.join(fitsdir,"ics_isotropic_skymap_"+galdefid))
				...
				if ( os.path.exists(os.path.join(fitsdir,"ics_isotropic_skymap_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"ics_isotropic_skymap_"+galdefid))
					self.gamma_rays["energy"] = 10**(float(sf[0].header["CRVAL3"]) + numpy.arange(int(sf[0].header["NAXIS3"]))*float(sf[0].header["CDELT3"]))
					b = float(sf[0].header["CRVAL2"]) + numpy.arange(int(sf[0].header["NAXIS2"]))*float(sf[0].header["CDELT2"])
					self.gamma_rays["IC"] = (sf[0].data.sum(axis=3)*numpy.sin(numpy.pi/2.-numpy.radians(b))).sum(axis=2)[0]/sf[0].data.shape[3]/numpy.sin(numpy.pi/2.-numpy.radians(b)).sum()/self.gamma_rays["energy"]**2
				if ( os.path.exists(os.path.join(fitsdir,"bremss_skymap_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"bremss_skymap_"+galdefid))
					self.gamma_rays["energy"] = 10**(float(sf[0].header["CRVAL3"]) + numpy.arange(int(sf[0].header["NAXIS3"]))*float(sf[0].header["CDELT3"]))
					b = float(sf[0].header["CRVAL2"]) + numpy.arange(int(sf[0].header["NAXIS2"]))*float(sf[0].header["CDELT2"])
					self.gamma_rays["bremss"] = (sf[0].data.sum(axis=3)*numpy.sin(numpy.pi/2.-numpy.radians(b))).sum(axis=2)[0]/sf[0].data.shape[3]/numpy.sin(numpy.pi/2.-numpy.radians(b)).sum()/self.gamma_rays["energy"]**2
				if ( os.path.exists(os.path.join(fitsdir,"pion_decay_skymap_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"pion_decay_skymap_"+galdefid))
					self.gamma_rays["energy"] = 10**(float(sf[0].header["CRVAL3"]) + numpy.arange(int(sf[0].header["NAXIS3"]))*float(sf[0].header["CDELT3"]))
					b = float(sf[0].header["CRVAL2"]) + numpy.arange(int(sf[0].header["NAXIS2"]))*float(sf[0].header["CDELT2"])
					self.gamma_rays["pion_decay"] = (sf[0].data.sum(axis=3)*numpy.sin(numpy.pi/2.-numpy.radians(b))).sum(axis=2)[0]/sf[0].data.shape[3]/numpy.sin(numpy.pi/2.-numpy.radians(b)).sum()/self.gamma_rays["energy"]**2
				...
				if ( os.path.exists(os.path.join(fitsdir,"ics_isotropic_mapcube_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"ics_isotropic_mapcube_"+galdefid))
					b = float(sf[0].header["CRVAL2"]) + (numpy.arange(int(sf[0].header["NAXIS2"]))-int(sf[0].header["CRPIX2"]))*float(sf[0].header["CDELT2"])
					self.gamma_rays["IC"] = (sf[0].data.sum(axis=2)*numpy.sin(numpy.pi/2.-numpy.radians(b))).sum(axis=1)/sf[0].data.shape[2]/numpy.sin(numpy.pi/2.-numpy.radians(b)).sum()
					self.gamma_rays["energy"] = sf[1].data.field(0)
				if ( os.path.exists(os.path.join(fitsdir,"bremss_mapcube_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"bremss_mapcube_"+galdefid))
					b = float(sf[0].header["CRVAL2"]) + (numpy.arange(int(sf[0].header["NAXIS2"]))-int(sf[0].header["CRPIX2"]))*float(sf[0].header["CDELT2"])
					self.gamma_rays["bremss"] = (sf[0].data.sum(axis=2)*numpy.sin(numpy.pi/2.-numpy.radians(b))).sum(axis=1)/sf[0].data.shape[2]/numpy.sin(numpy.pi/2.-numpy.radians(b)).sum()
					self.gamma_rays["energy"] = sf[1].data.field(0)
				if ( os.path.exists(os.path.join(fitsdir,"pion_decay_mapcube_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"pion_decay_mapcube_"+galdefid))
					b = float(sf[0].header["CRVAL2"]) + (numpy.arange(int(sf[0].header["NAXIS2"]))-int(sf[0].header["CRPIX2"]))*float(sf[0].header["CDELT2"])
					self.gamma_rays["pion_decay"] = (sf[0].data.sum(axis=2)*numpy.sin(numpy.pi/2.-numpy.radians(b))).sum(axis=1)/sf[0].data.shape[2]/numpy.sin(numpy.pi/2.-numpy.radians(b)).sum()
					self.gamma_rays["energy"] = sf[1].data.field(0)
			if ( os.path.exists(os.path.join(fitsdir,"ics_isotropic_healpix_"+galdefid))
				...
				if ( os.path.exists(os.path.join(fitsdir,"ics_isotropic_healpix_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"ics_isotropic_healpix_"+galdefid))
					self.gamma_rays["IC"] = sf[1].data.field(0).sum(axis=0)/float(sf[1].header["NAXIS2"])
					self.gamma_rays["energy"] = sf[2].data.field(0)
				if ( os.path.exists(os.path.join(fitsdir,"bremss_healpix_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"bremss_healpix_"+galdefid))
					self.gamma_rays["bremss"] = sf[1].data.field(0).sum(axis=0)/float(sf[1].header["NAXIS2"])
					self.gamma_rays["energy"] = sf[2].data.field(0)
				if ( os.path.exists(os.path.join(fitsdir,"pi0_decay_healpix_"+galdefid)) ):
					sf = pyfits.open(os.path.join(fitsdir,"pi0_decay_healpix_"+galdefid))
					self.gamma_rays["pion_decay"] = sf[1].data.field(0).sum(axis=0)/float(sf[1].header["NAXIS2"])
					self.gamma_rays["energy"] = sf[2].data.field(0)

			if ("energy" in self.gamma_rays):
				self.gamma_rays["total"] = numpy.zeros(self.gamma_rays["energy"].shape)
...
# Unfortunately there is no way to tell which is which from the nuclei file.
	def CRspectra(self,Z, A, K=-1, joined=True, phi=0, out_energy=[]):
		...
		out_brdn = numpy.zeros((len(spectra_brdn),len(out_energy)), dtype='float')
			...
			for (i,en) in enumerate(energy):
				if (en >= self.energy[0] and en <= self.energy[-1]):
					...
					if (spectra_brdn[k][j-1] > 0 and spectra_brdn[k][j] > 0):
						sl = math.log(spectra_brdn[k][j-1]/spectra_brdn[k][j])/math.log(self.energy[j-1]/self.energy[j])
						out_brdn[k,i] = spectra_brdn[k][j-1]*(en/self.energy[j-1])**sl
					elif (spectra_brdn[k][j-1] > 0):
						out_brdn[k,i] = (self.energy[j]-en)*spectra_brdn[k][j-1]/(self.energy[j]-self.energy[j-1])
					elif (spectra_brdn[k][j] > 0):
						out_brdn[k,i] = (en - self.energy[j-1])*spectra_brdn[k][j]/(self.energy[j]-self.energy[j-1])
					...
					out_brdn[k,i] *= out_energy[i]*(out_energy[i]+2.*m)/(en*(en+2.*m))
	...
	def CRIsotopes(self,energy,phi=0):
		...
		out = []
		for Z,vZ in self.density.iteritems():
			for A, vA in vZ.iteritems():
...
def require(condition):
	if (condition != True):
		...
		print "Usage: ", argv[0], " fitsdir galdefid parser [arg1 [arg2 [ ... ]]]"
		...
		print "Example: ", argv[0], " ./my_results 54_00010001.gz spectra 250 2 4 2.7"
		print "         will read ./my_results/nuclei_54_00010001.gz and output the spectrum"
...
def gplot(*argv):
    ...
    if (parser == "spectra"):
        ...
        resultGP=numpy.zeros(2*len(energy),dtype=object)
        spectra *= energy**alpha
        # print "# Energy spectrum of a cosmic ray isotope calculated in GALPROP run # %s"%galdefid
        # print "# for Z=%3d, A=%3d, V=%10.3e MV, R=%6.2f kpc. "%(Z, A, phi, gdsp.Rsun)
        ...
        #typeres0=type(resultGP[0])
        
        msg="Energy spectrum for Z=%3d, A=%3d, V=%10.3e MV, R=%6.2f kpc in GALPROP run # %s"%(Z, A, phi, gdsp.Rsun,galdefid)
        #print msg
        
        for k in range(len(energy)):
            str = "%12.4e %12.4e " %(energy[k], spectra[0][k])
            resultGP[k]=numpy.zeros(1+int(spectra.shape[0]))
            resultGP[k][0]=energy[k]
            resultGP[k][1]=spectra[0][k]
            
            for j in range(1,spectra.shape[0]):
                str += "%12.4 " % spectra[j][k]
                resultGP[k][j+1]=spectra[j][k]
            #print str
        return msg,resultGP
    elif (parser == "ratios"):
        ...
        for A1 in A1list:
            #print(int(A1))
            (energy,spectra_breakdown,type) = gdsp.CRspectra(Z1,int(A1),joined=True,phi=phi)
            if (n_found_1==0):
                if (spectra_breakdown[0][0]>0): # This checks for non-existent isotopes
        ...
        for A2 in A2list:
            #print(int(A2))
            (energy,spectra_breakdown,type) = gdsp.CRspectra(Z2,int(A2),joined=True,phi=phi)
            if (n_found_2==0):
                if (spectra_breakdown[0][0]>0):
        ...
        # print "# for Z1=%3d, A1=%s, Z2=%3d, A2=%s, V=%10.3e MV, R=%6.2f kpc. "%(Z1, A1str, Z2, A2str, phi, gdsp.Rsun)
        # print "# The first column is E, the kinetic energy per nucleon after modulation, in MeV,"
        # print "# and the second column contains the dimensionless ratio of (Z1; A1) to (Z2; A2)."
        # print "# %10s %12s"%("Energy", "Ratio")
        
        msg="Flux ratios cosmic ray isotopes for Z1=%3d, A1=%s, Z2=%3d, A2=%s, V=%10.3e MV, R=%6.2f kpc. "%(Z1, A1str, Z2, A2str, phi, gdsp.Rsun)
        resultGP=numpy.zeros(2*len(energy),dtype=object)
        for k in range(len(energy)):
            resultGP[k]=numpy.zeros(1+int(spectra1.shape[0]))
            resultGP[k][0]=energy[k]
            if ((spectra1[0][k]>0) and (spectra2[0][k]>0)):
                str = "%12.4e %12.4e " %(energy[k], spectra1[0][k]/spectra2[0][k])
                resultGP[k][1]=spectra1[0][k]/spectra2[0][k]
                for j in range(1,spectra1.shape[0]):
                    str += "%12.4 " % (spectra1[j][k]/spectra2[j][k])
                    resultGP[k][j+1]=spectra1[j][k]/spectra2[j][k]
                #print str
        return msg,resultGP
        ...
        print "# for E=%10.3e MeV/nucleon, V=%10.3e MV, R=%6.2f kpc"%(E, phi, gdsp.Rsun)
        ...
        if "energy" in gdsp.gamma_rays:
            ...
            cmp.remove("energy")
            cmp.remove("total")
            print "# Gamma-ray emission spectra calculated in GALPROP run # %s"%galdefid
            print "# The spectra represent the averages over the region of the sky specified in the GALDEF file,"
            print "# or over the whole sky, if the HEALPIX format was used for skymaps."
            print "# The first column if the photon energy E in MeV, and columns 2, 3, 4 and 5 represent"
            print "# the total, pion decay, inverse Compton and Bremsstrahlung spectra, respectively."
            print "# These spectra are multiplied by E^%4.2f and have the units of MeV^%4.2f*cm^-2*s^-1*sr^-1*MeV^-1."%(alpha, alpha)
            str = "#Energy [MeV]  total flux*E^%4.2f [MeV^%4.2f cm^-2 s^-1 sr^-1 MeV^-1]"%(alpha,alpha)
            for c in cmp:
                str += " "+c
            print str
            print "# %10s %12s %12s %12s %12s"%("E", "Total", "Pion Decay", "Inv. Compt.", "Bremsstr.")
            for j in range(len(gdsp.gamma_rays["energy"])):
                str = "%12.4e %12.4e "%(gdsp.gamma_rays["energy"][j],(gdsp.gamma_rays["total"][j]*(gdsp.gamma_rays["energy"][j])**alpha))
                str += "%12.4e"%(gdsp.gamma_rays["pion_decay"][j]*(gdsp.gamma_rays["energy"][j])**alpha)
                str += "%12.4e"%(gdsp.gamma_rays["IC"][j]*(gdsp.gamma_rays["energy"][j])**alpha)
                str += "%12.4e"%(gdsp.gamma_rays["bremss"][j]*(gdsp.gamma_rays["energy"][j])**alpha)
            print str
        ...
        if "total" in gdsp.synchrotron:
            print "# Synchrotron radiation spectrum calculated in GALPROP run # %s"%galdefid
            print "# The spectrum represents the average over the region of the sky specified in the GALDEF file,"
            print "# or over the whole sky, if the HEALPIX format was used for skymaps."
            print "# The first column in the wave frequency f in Hz, and the second column is the"
            ...
            for j in range(len(gdsp.synchrotron["total"])):
                print "%12.4e %12.4e"%(gdsp.synchrotron["nu"][j], (gdsp.synchrotron["total"][j])*(gdsp.synchrotron["nu"][j])**alpha)